...
def validate_frozen_topic_roster(
    ...
    if any(topic["lifecycle_status"] != "frozen" for topic in topics.values()):
        raise ValueError("frozen topic set 中每个 topic lifecycle_status 都必须 frozen。")
...
def validate_topic_research_artifact(
    ...
    *,
    ...
    if payload["status"] != "frozen" or payload["selection_frozen_before_labels"] is not True:
        raise ValueError("Topic research 必须在 labels 前 frozen。")
    _require_datetime(payload["created_at"], "topic research created_at")
    criteria = _require_string_list(payload["criteria"], "topic criteria", nonempty=True)
    required_criteria = {
        "scientific_object",
        "data_modality",
        "target_task",
    ...
    for raw in _require_nonempty_list(payload["viability_evidence"], "viability evidence"):
        ...
        if candidate_id not in candidates:
            raise ValueError(f"viability evidence 引用 unknown candidate：{candidate_id}。")
        ...
        for work in works:
            ...
            _require_nonempty_string(work_map["openalex_id"], "representative openalex_id")
            _require_nonempty_string(work_map["title"], "representative title")
            if isinstance(work_map["year"], bool) or not isinstance(work_map["year"], int):
    ...
    selected = set(
        _require_string_list(freeze["selected_candidate_ids"], "selected candidates", nonempty=True)
    )
    excluded = set(
        _require_string_list(freeze["excluded_candidate_ids"], "excluded candidates", nonempty=True)
    )
    ...
    final_topic_ids = set(
        _require_string_list(freeze["final_topic_ids"], "final topic IDs", nonempty=True)
    )
    candidate_topic_ids = {
    ...
    if split is not None:
        if split_decision["dev_topic_ids"] != split["dev_topic_ids"] or split_decision[
            "hidden_test_topic_ids"
        ] != split["hidden_test_topic_ids"]:
            raise ValueError("research split decision 与 frozen split artifact 不一致。")
...
def validate_topic_freeze_files(
    ...
    *,
    ...
    if research_path is not None:
        ...
        validate_topic_research_artifact(research, topics=topics, split=split_payload)
        if _parse_datetime(split_payload["frozen_at"]) < _parse_datetime(
...
def validate_annotation_protocol(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    if payload["status"] != "frozen" or payload["frozen_before_annotations"] is not True:
    ...
    if payload["label_scheme"] != {
        ...
        raise ValueError("annotation protocol 必须使用 W4-compatible 0/1/2 Query Relevance。")
    ...
    visible = set(_require_string_list(blind["visible_fields"], "blind visible fields", nonempty=True))
    if not {
        "research_question",
        "scope_in",
        "scope_out",
    ...
    forbidden = set(
        _require_string_list(blind["forbidden_signals"], "blind forbidden signals", nonempty=True)
    )
    if not set(BLIND_TASK_FORBIDDEN_KEYS) <= forbidden:
        raise ValueError("blind protocol 未覆盖 Bootstrap retrieval/ranking leakage keys。")
    ...
    _require_string_list(evidence["source_priority"], "evidence source priority", nonempty=True)
    required_result_fields = set(
        _require_string_list(
            evidence["required_result_fields"], "evidence required fields", nonempty=True
        )
    )
    if not {
        "label",
    ...
    _require_fraction(review["second_annotation_fraction_per_topic"], "second annotation fraction")
    _require_fraction(review["random_high_confidence_qa_fraction"], "random QA fraction")
    ...
    adjudication = _require_mapping(payload["adjudication_policy"], "adjudication policy")
    _require_exact_fields(
        adjudication,
        {
    ...
    if adjudication["required_for_conflicts"] is not True or adjudication[
        "preserve_primary_history"
    ] is not True:
...
def select_second_annotation_tasks(
    *,
    ...
    for topic_id in sorted(by_topic):
        ...
        count = math.ceil(len(task_ids) * fraction_per_topic) if fraction_per_topic else 0
        ranked = sorted(
            task_ids,
            key=lambda task_id: (
...
def build_review_plan_payload(
    *,
    ...
    conflicting_annotation_ids: Iterable[str] = (),
) -> dict[str, Any]:
    ...
    conflict_ids = set(conflicting_annotation_ids)
    unknown = (boundary_ids | conflict_ids | set(lookup)).difference(annotations)
    if unknown:
        raise ValueError("review plan signal 引用 unknown annotation：" + ", ".join(sorted(unknown)) + "。")
    ...
    high_confidence = [
        annotation_id
        for annotation_id, annotation in annotations.items()
        if annotation["confidence"] == "high" and not reasons_by_annotation[annotation_id]
    ]
    ...
    review_by_annotation = {review["annotation_id"]: review for review in reviews.values()}
    review_items = []
    for annotation_id in sorted(
        annotation_id for annotation_id, reasons in reasons_by_annotation.items() if reasons
    ):
        ...
        if review is not None:
            ...
            if _parse_datetime(review["reviewed_at"]) < _parse_datetime(
                annotation["annotation_provenance"]["created_at"]
            ):
                raise ValueError(f"review {review['review_id']} 时间早于 primary annotation。")
        review_items.append(
            {
    ...
    missing = [item["annotation_id"] for item in review_items if item["review_status"] == "pending"]
    payload = {
        "schema_version": W6_SCHEMA_VERSION,
        "artifact_type": REVIEW_PLAN_ARTIFACT_TYPE,
        "artifact_id": "w6_annotation_review_plan_v1",
        "status": "in_review" if missing else "review_complete",
...
def validate_review_plan(
    ...
    *,
    ...
    if payload["protocol"] != dict(protocol_reference) or payload["annotations"] != dict(
    ...
    lookup = {
        ...
        for item in _require_list(payload["evidence_lookup_statuses"], "lookup statuses")
    }
    expected = build_review_plan_payload(
        ...
        conflicting_annotation_ids=payload["conflicting_annotation_ids"],
    )
    if payload != expected:
...
def build_w6_benchmark_package(
    *,
    ...
    if status != "bootstrap_fixture" and is_fixture:
        raise ValueError("synthetic fixture 不得冒充 draft/proposed/sealed research Benchmark。")
...
def validate_w6_benchmark_package(package_path: str | Path) -> dict[str, Any]:
    ...
    if package["status"] not in {"bootstrap_fixture", "draft", "proposed", "sealed_candidate"}:
        raise ValueError("benchmark package status 非法。")
    ...
    if generation.get("git_worktree_clean") is not True or generation.get(
        "labels_used_for_generation"
    ) is not False:
        raise ValueError("Benchmark builder 必须 clean 且不得用 labels 生成 inputs/config。")
    _require_git_revision(generation.get("git_revision"), "benchmark generation git_revision")
    ...
    for name, raw in artifact_entries.items():
        ...
        _require_exact_fields(entry, {"artifact_id", "path", "sha256"}, f"package artifact {name}")
    ...
    if benchmark["status"] != package["status"] or benchmark["is_fixture"] != package[
        "is_fixture"
    ]:
        raise ValueError("package 与 Benchmark manifest status/fixture identity 不一致。")
...
def _load_and_validate_graph(
    ...
    *,
    ...
    payloads = {name: load_json_object(path, label=name) for name, path in paths.items()}
    ...
    retrieval = validate_retrieval_provenance(payloads["retrieval_provenance"], topics=topics)
    ...
    reviews = validate_annotation_reviews(payloads["annotation_reviews"], annotations=annotations)
    validate_hidden_label_anchor(
...
def _build_benchmark_manifest(
    *,
    ...
    review_status = "in_review" if review_plan["coverage"]["missing_review_annotation_ids"] else "not_started"
    benchmark = {
        "schema_version": W6_SCHEMA_VERSION,
...
def validate_benchmark_status_gate(
    ...
    if annotated_pool_ids != dev_pool_ids:
        raise ValueError("proposed/sealed_candidate 必须完整覆盖 Dev Candidate Pool annotations。")
    if status == "sealed_candidate" and review_plan["coverage"]["missing_review_annotation_ids"]:
