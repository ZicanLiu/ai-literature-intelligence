...
`app/main.py` 读取 `--mode`、`--keyword` 和 `--max-results`，并把这些参数传给后续模块。
...
被去重的记录会保存到 `outputs/tables/duplicates_removed.csv`，其中包含 `duplicate_reason`。
...
SQLite 只使用 Python 自带的 `sqlite3`，第一版只建一张 `papers` 表，方便初学者理解。
