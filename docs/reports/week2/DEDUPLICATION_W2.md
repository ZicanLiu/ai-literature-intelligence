...
**测试数据:** 21 条 fixture + 38 个自动化测试
**结果生成命令:** `python -m app.review_duplicates --generate`
...
两对疑似重复均来自 `run001_ml_stellar_spectra` 同一批次，不是跨 run 配对。每对标题
标准化后高度一致，但 `openalex_id` 不同，且 DOI 一侧有值、一侧为空；可能与版本或
元数据差异有关，需要人工复核，现有数据不足以解释 OpenAlex ID 不同的内部原因。

- **SP-8d2fce71**：两条记录都来自 `run001`，标题大小写不同，年份差 1 年
  （2014 vs 2015）。**建议人工确认。**
- **SP-fe25e871**：两条记录都来自 `run001`，标题大小写不同，年份一致（2005）。
  **建议人工确认。**
...
### 案例 1：标题高度一致但 ID 不同
两对记录的标准化标题完全相同（相似度 1.000），作者也完全相同，但 OpenAlex ID
不同。当前只能确认它们需要进入人工复核队列，不能据此推断 OpenAlex 的 ID 生成原因。
...
|---------|:---:|
| 相同 OpenAlex ID | ✓ |
| 相同 DOI（URL 前缀差异，检查具体 merged_id） | ✓ |
| 标题完全相同（双方无 ID 无 DOI → 确定重复） | ✓ |
| 不同 OpenAlex ID 相同标题（不自动合并） | ✓ |
| 标点差异进入疑似队列（检查具体 ID） | ✓ |
| 标题带副标题/arXiv ID | ✓ |
| 年份相差 1 年（检查具体 ID） | ✓ |
| 作者明显不同 | ✓ |
...
| 确定/疑似严格分开 | ✓ |
| DOI 标准化（多前缀） | ✓ |
| 作者姓氏提取 + 重合度（含双方空列表 → 0.0） | ✓ |
| Jaccard 边界值（0, 0.5, 1.0, 双方空 → 0.0） | ✓ |
| SequenceMatcher 边界值（含单方空/双方空 → 0.0） | ✓ |
| `--generate` 非交互结果生成命令 | ✓ |
| `--list` 列表命令不触发 `args.stats` 错误 | ✓ |
| `--stats` 审核统计命令 | ✓ |
| 未知参数由 argparse 返回错误 | ✓ |
